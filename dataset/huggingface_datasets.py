...
from dataset.st_dataset import (
    SummInstance,
    SummDataset,
    generate_train_dev_test_splits,
...
class MultinewsDataset(HuggingfaceDataset):
    ...
    def __init__(self):
        ...
        processed_train_set = MultinewsDataset.process_multinews_data(
            multinews_dataset["train"]
        )
        processed_dev_set = MultinewsDataset.process_multinews_data(
            multinews_dataset["validation"]
        )
        processed_test_set = MultinewsDataset.process_multinews_data(
            multinews_dataset["test"]
        )
...
class SamsumDataset(HuggingfaceDataset):
    ...
    def __init__(self):
        ...
        processed_dev_set = SamsumDataset.process_samsum_data(
...
class PubmedqaDataset(HuggingfaceDataset):
    ...
    def __init__(self, seed=None):
        ...
        pubmedqa_split_dataset = generate_train_dev_test_splits(
            pubmedqa_dataset["train"], seed=seed
        )

        processed_train_set = PubmedqaDataset.process_pubmedqa_data(
            pubmedqa_split_dataset["train"]
        )
        processed_dev_set = PubmedqaDataset.process_pubmedqa_data(
            pubmedqa_split_dataset["dev"]
        )
        processed_test_set = PubmedqaDataset.process_pubmedqa_data(
            pubmedqa_split_dataset["test"]
        )
